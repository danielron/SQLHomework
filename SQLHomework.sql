...
SELECT last_name, Count(last_name) AS CountOflast_name
FROM actor
GROUP BY last_name;
...
FROM staff LEFT JOIN address ON staff.address_id = address.address_id LEFT JOIN city ON address.city_id = city.city_id LEFT JOIN country ON city.country_id = country.country_id;
...
SELECT film.title, Count(inventory.inventory_id) AS CopyCount
...
FROM customer LEFT JOIN address ON customer.address_id = address.address_id LEFT JOIN city ON address.city_id = city.city_id LEFT JOIN country ON city.country_id = country.country_id where country.country = "Canada";
...
SELECT category.name AS FilmGenre, SUM(payment.amount)
FROM category
JOIN film_category
ON(film_category.category_id = category.category_id)
JOIN inventory
